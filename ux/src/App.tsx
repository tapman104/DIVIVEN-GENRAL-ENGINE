...
import { applyMove, cloneState } from './engine/board';
...
function App() {
  ...
  useEffect(() => {
    ...
    // Sync lite mode on init
    bridge.current.configureEngine({ liteMode: isLiteMode });
  ...
  return (
    <div className={`app-container ${isLiteMode ? 'lite-mode' : ''}`} style={{ '--accent-color': themeColor, '--accent-rgb': themeRgb } as React.CSSProperties}>
          ...
          <aside className="control-panel">
            ...
            <div className="actions">
              ...
              <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                ...
                <button
                  className={`btn-ghost ${isLiteMode ? 'active' : ''}`}
                  onClick={toggleLiteMode}
                  style={isLiteMode ? { borderColor: 'var(--accent-color)', color: 'var(--accent-color)' } : {}}
                >
                  {isLiteMode ? 'LITE: ON' : 'LITE: OFF'}
                </button>
              </div>
            </div>
          </aside>
