...
exports.generateZobristKey = generateZobristKey;
// Random values for each piece on each square
// [color][pieceType][rank][file]
const PIECE_KEYS = [];
const TURN_KEY = Math.floor(Math.random() * Number.MAX_SAFE_INTEGER);
const CASTLING_KEYS = []; // 4 keys
const EN_PASSANT_KEYS = []; // 8 keys (one for each file)
...
for (let i = 0; i < 4; i++)
    CASTLING_KEYS[i] = Math.floor(Math.random() * Number.MAX_SAFE_INTEGER);
for (let i = 0; i < 8; i++)
    EN_PASSANT_KEYS[i] = Math.floor(Math.random() * Number.MAX_SAFE_INTEGER);
function generateZobristKey(state) {
    let key = BigInt(0);
    // 1. Pieces
    for (let r = 0; r < 8; r++) {
        for (let f = 0; f < 8; f++) {
